...
export const EventBlock: React.FC<EventBlockProps> = ({ event, onClick, style, stickyLeft, zoomLevel }) => {
  ...
  if (zoomLevel === 0) { // Minimalistic view
    return (
      <div
        ...
        className={`absolute top-1/2 -translate-y-1/2 rounded-md cursor-pointer transition-transform hover:scale-105 shadow-sm ${bg} ${border} border overflow-hidden`}
        style={dynamicStyle}
  ...
  return (
    <div
      onClick={() => onClick(event)}
      className={`absolute top-1/2 -translate-y-1/2 rounded-lg cursor-pointer transition-transform hover:scale-105 shadow-sm ${bg} ${border} border overflow-hidden`}
      style={dynamicStyle}
    >
      <div
