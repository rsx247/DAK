...
import type { FilterState, TimeViewMode, FoodEvent, Venue, VenueSubmissionData, EventSubmissionData, EventData, UserLocation, SortByType, AccessLevel } from './types';
...
const App: React.FC = () => {
    ...
    const [filters, setFilters] = useState<FilterState>({ distance: 5, maxCost: Infinity, accessLevels: [], eventTypes: [], dietaryNeeds: [] });
    ...
    const renderContent = () => {
        ...
        switch (viewMode) {
            case 'TIMELINE':
                return <TimelineGrid venues={venues} eventsByVenue={eventsByVenue} currentTime={currentTime} onEventClick={setSelectedEvent} venueCount={venues.length} />;
            case 'LIST':
