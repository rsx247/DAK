...
export const EventBlock: React.FC<EventBlockProps> = ({ event, onClick, style }) => {
  const { bg, border } = categoryColors[event.venue.category] || { bg: 'bg-gray-50', border: 'border-gray-200' };
  
  return (
    <div
      onClick={() => onClick(event)}
      className={`absolute top-1/2 -translate-y-1/2 h-[calc(100%-1.25rem)] p-2 rounded-lg cursor-pointer transition-transform hover:scale-105 shadow-sm ${bg} ${border} border flex flex-col justify-center overflow-hidden`}
      style={style}
    >
